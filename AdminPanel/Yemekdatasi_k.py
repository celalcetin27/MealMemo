...
class Yemekdatapage(QWidget):
    def __init__(self) -> None:
        ...
        self.Yemekdatapage_e.pushButton_3.clicked.connect(self.adetsilme)
    ...
    def adetsilme(self):
        klasor_adi = f"{self.selected_text}_{self.yemekismi}"
        try:
            os.makedirs(klasor_adi, exist_ok=True)
            
            dosyalar = [dosya for dosya in os.listdir(klasor_adi) if dosya.endswith(".jpg")]
            dosyalar.sort()  # Dosya adlarını sırala
            
            if not dosyalar:
                print(f"{klasor_adi} klasöründe jpg dosyası bulunamadı.")
                return
            
            dosya_silinecek = int(self.Yemekdatapage_e.spinBox_Adet.value())
            
            if 1 <= dosya_silinecek <= len(dosyalar):
                dosya_adı = dosyalar[dosya_silinecek - 1]
                dosya_yolu = os.path.join(klasor_adi, dosya_adı)
                
                os.remove(dosya_yolu)
                print(f"{dosya_adı} başarıyla silindi!")
            else:
                print("Geçersiz dosya numarası. Lütfen 1 ile dosya sayısı arasında bir değer girin.")
